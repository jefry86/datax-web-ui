    ...
    >
      ...
      <el-table-column align="center" label="任务名称" width="150">
        <template slot-scope="scope">
          <a :href="'#/datax/job/jobInfo?id='+scope.row.jobId" style="color: #0a76a4"> {{ scope.row.jobDesc }}</a>
        </template>
      </el-table-column>
      ...
      <el-table-column label="调度结果" align="center">
        <template slot-scope="scope"><span :style="`color:${scope.row.triggerCode==500?'red':''}`">{{ statusList.find(t => t.value === scope.row.triggerCode).label }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="执行时间" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.processId!=null">{{ scope.row.handleTime }}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column label="执行结果" align="center">
        <template slot-scope="scope">
          <p v-if="scope.row.processId!=null">
            <span :style="`color:${scope.row.handleCode==500?'red':''}`">{{ statusList.find(t => t.value === scope.row.handleCode).label }}</span>
          </p>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column label="消耗时间" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.handleCode!=null">{{ scope.row.expTime/1000 }}s</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
